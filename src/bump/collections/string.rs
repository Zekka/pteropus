...
use crate::bump::collections::vec::Vec;
...
macro_rules! format {
    ( in $bump:expr, $fmt:expr, $($args:expr),* ) => {{
        use std::fmt::Write;
        let bump = $bump;
        let mut s = $crate::bump::collections::String::new_in(bump);
...
impl<'bump> String<'bump> {
    ...
    /// ```
    #[inline]
    pub fn with_capacity_in(bump: &'bump Bump, capacity: usize) -> String<'bump> {
        String {
            vec: Vec::with_capacity_in(bump, capacity),
        }
    }
    ...
    pub fn from_utf8_lossy_in(v: &[u8], bump: &'bump Bump) -> String<'bump> {
        ...
        let mut res = String::with_capacity_in(bump, v.len());
        res.push_str(bump, first_valid);
        if !first_broken.is_empty() {
            res.push_str(bump, REPLACEMENT);
        }

        for lossy::Utf8LossyChunk { valid, broken } in iter {
            res.push_str(bump, valid);
            if !broken.is_empty() {
                res.push_str(bump, REPLACEMENT);
            }
        }
    ...
    /// assert!(String::from_utf16_in(v, &b).is_err());
    /// ```
    pub fn from_utf16_in(bump: &'bump Bump, v: &[u16]) -> Result<String<'bump>, FromUtf16Error> {
        let mut ret = String::with_capacity_in(bump, v.len());
    ...
    /// assert_eq!(s, "hello");
    /// ```
    pub fn from_str_in(bump: &'bump Bump, s: &str) -> String<'bump> {
        let mut t = String::with_capacity_in(bump, s.len());
        t.push_str(bump, s);
        t
    }
    ...
    /// ```
    #[inline]
    pub fn push_str(&mut self, bump: &'bump Bump, string: &str) {
        self.vec.extend_from_slice(bump, string.as_bytes())
    }
    ...
    pub fn reserve(&mut self, bump: &'bump Bump, additional: usize) {
        self.vec.reserve(bump, additional)
    }
    ...
    pub fn reserve_exact(&mut self, bump: &'bump Bump, additional: usize) {
        self.vec.reserve_exact(bump, additional)
    }
    ...
    /// ```
    #[inline]
    pub fn push(&mut self, bump: &'bump Bump, ch: char) {
        match ch.len_utf8() {
            1 => self.vec.push(bump, ch as u8),
            _ => self
                .vec
                .extend_from_slice(bump, ch.encode_utf8(&mut [0; 4]).as_bytes()),
        }
    }
    ...
    /// ```
    #[inline]
    pub fn insert(&mut self, bump: &'bump Bump, idx: usize, ch: char) {
        assert!(self.is_char_boundary(idx));
        let mut bits = [0; 4];
        let bits = ch.encode_utf8(&mut bits).as_bytes();

        unsafe {
            self.insert_bytes(bump, idx, bits);
        }
    }

    unsafe fn insert_bytes(&mut self, bump: &'bump Bump, idx: usize, bytes: &[u8]) {
        let len = self.len();
        let amt = bytes.len();
        self.vec.reserve(bump, amt);
    ...
    /// ```
    #[inline]
    pub fn insert_str(&mut self, bump: &'bump Bump, idx: usize, string: &str) {
        assert!(self.is_char_boundary(idx));

        unsafe {
            self.insert_bytes(bump, idx, string.as_bytes());
        }
    }
    ...
    /// ```
    #[inline]
    pub fn split_off(&mut self, bump: &'bump Bump, at: usize) -> String<'bump> {
        assert!(self.is_char_boundary(at));
        let other = self.vec.split_off(bump, at);
...
impl<'bump> BumpExtend<'bump, char> for String<'bump> {
    fn extend<I: IntoIterator<Item = char>>(&mut self, bump: &'bump Bump, iter: I) {
        let iterator = iter.into_iter();
        let (lower_bound, _) = iterator.size_hint();
        self.reserve(bump, lower_bound);
        for ch in iterator {
            self.push(bump, ch)
        }
    }
...
impl<'a, 'bump> BumpExtend<'bump, &'a char> for String<'bump> {
    fn extend<I: IntoIterator<Item = &'a char>>(&mut self, bump: &'bump Bump, iter: I) {
        self.extend(bump, iter.into_iter().cloned());
    }
}

impl<'a, 'bump> BumpExtend<'bump, &'a str> for String<'bump> {
    fn extend<I: IntoIterator<Item = &'a str>>(&mut self, bump: &'bump Bump, iter: I) {
        for s in iter {
            self.push_str(bump, s)
        }
    }
}

impl<'bump> BumpExtend<'bump, String<'bump>> for String<'bump> {
    fn extend<I: IntoIterator<Item = String<'bump>>>(&mut self, bump: &'bump Bump, iter: I) {
        for s in iter {
            self.push_str(bump, &s)
        }
    }
...
impl<'bump> BumpExtend<'bump, ::std::string::String> for String<'bump> {
    fn extend<I: IntoIterator<Item = ::std::string::String>>(&mut self, bump: &'bump Bump, iter: I) {
        for s in iter {
            self.push_str(bump, &s)
        }
    }
...
impl<'a, 'bump> BumpExtend<'bump, Cow<'a, str>> for String<'bump> {
    fn extend<I: IntoIterator<Item = Cow<'a, str>>>(&mut self, bump: &'bump Bump, iter: I) {
        for s in iter {
            self.push_str(bump, &s)
        }
    }
